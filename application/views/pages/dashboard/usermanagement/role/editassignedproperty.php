  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Update User Property Assign</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Update User property Assign</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        <!-- SELECT2 EXAMPLE -->
          ...
          <div class="card-header">
            <h3 class="card-title">Update User Property Assign</h3>
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
							<form method="post" action="<?php echo base_url('portal/users/assignupdate'); ?>">
							
							<!-- <input type="hidden" name="uap_cid" value="<?php echo $_SESSION['passed_user_national'] ?>" />
							<input type="hidden" name="uap_cdate" value="<?php echo $date; ?>" /> -->
							<input type="hidden" name="uap_udate" value="<?php echo $date; ?>" />
							<input type="hidden" name="uap_uid" value="<?php echo $_SESSION['passed_user_national'] ?>"  />
...
for($i = 0 ; $i < count($_SESSION['user_nic_list1']) ; $i++) 
			 {                                                  
echo '<option value="'.$_SESSION['user_role_list1'][$i].'">'.$_SESSION['user_nic_list1'][$i].'</option>';
			 }
				...
				<div class="form-group">
					<label>Select User</label>
					<select id="get_user"
                           name="get_user"  onchange="load_director()"  class="form-control select2" style="width: 100%;">								
...
echo '<option value="'.$_SESSION['user_nic_list'][$i].'">'.$_SESSION['user_nic_list'][$i].'</option>';
				 ...
				 ?>
					...
					<label>Select property</label>
                  		<select name="selected_role1" class="form-control select2" style="width: 100%;">
                    	<?php 

for($i = 0 ; $i < count($_SESSION['user_property_id_list']) ; $i++) 
			 {                                                  
echo '<option value="'.$_SESSION['user_property_id_list'][$i].'">'.$_SESSION['user_property_name_list'][$i].'</option>';
			 }
              ...
              <div class="col-md-6">
				...
				<label>Assigned Property Name</label>
                        <input type="text" name="user_role" id="user_role" class="form-control" placeholder="" disabled>
								...
								<br><br><br><br>
											<div  align='right'>
											<button type="submit" value="assign_role" name="assign_role"  class="btn btn-warning">Update Assign</button>
											</div>
                </div>
                <!-- /.form-group -->
              </div>
...
function load_director(){   
                ...
                var get_user = document.getElementById('get_user').value;
                        ...
                        var url = '<?php echo base_url('portal/users/load_role1'); ?>';
